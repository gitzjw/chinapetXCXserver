...
Class MYDB {
    ...
    public function __construct() {
        ...
        if($_SERVER['HTTP_HOST'] != $release_domain){
            include 'configs.inc.beta.php';
        }else{
            require "configs.inc.php";
        }
        $this->connect($configs["hostname"], $configs["username"], $configs["password"], $configs["database"], $configs["dbcharset"]);
    }
     
    //数据库连接
    public function connect($dbhost, $dbuser, $dbpw, $dbname, $charset='utf8',$pconnect=0) {
        if( $pconnect==0 ) {
